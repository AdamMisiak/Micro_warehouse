...
@router.get("/{sku}", response_model=models.ProductWithBatches, tags=["products"])
def get_product(sku: str, db: Session = Depends(get_db)):
    product = services.get_product(sku=sku, repository=repository.SqlAlchemyRepository(session=db))
    if product is None:
        raise HTTPException(status_code=404, detail="Product not found")
    return product


@router.get("/", response_model=List[models.ProductWithBatches], tags=["products"])
